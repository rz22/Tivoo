...
public class TivooFilter {
	...
	public static List<TivooEvent> filterTime(List<TivooEvent> eventlist,
		...
		for (TivooEvent e : eventlist) {
			...
			if (!(comp.compare(end, eventstart) < 0 || comp.compare(eventend,
					start) < 0))
				filtered.add(e);
		}
		return filtered;
	...
	public static List<TivooEvent> filterSpecial(List<TivooEvent> eventlist,
			Set<String> keywords, TivooEventType eventtype, String attr,
			boolean reverse) {
		List<TivooEvent> filtered = new ArrayList<TivooEvent>();
		for (TivooEvent e : eventlist) {
			int count = 0;
