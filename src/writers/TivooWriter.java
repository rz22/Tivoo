...
public abstract class TivooWriter {
	...
	protected abstract void writeEvents(HtmlCanvas target,
			List<TivooEvent> eventlist, String summarypath) throws IOException;
	...
	private void doWrite(List<TivooEvent> eventlist, String path)
			throws IOException {
	...
	protected String buildDetailPathRel(List<TivooEvent> eventlist,
			TivooEvent e, String summarypath) {
		// System.out.println("Event is: " + e);
		return "details/"
				.concat(e.getTitle().replaceAll("[^A-z0-9]", "")
						.replaceAll("\\s+", "_").trim()
						.concat(Integer.toString(eventlist.indexOf(e)))
						.concat(".html"));
	}
	...
	protected HtmlCanvas startPage(FileWriter fw, List<TivooEvent> eventlist,
	...
	protected void writeHeadWithCSS(HtmlCanvas target, String stylefile)
			throws IOException {
		target.head()
				.link(href(stylefile).type("text/css").rel("stylesheet")
						.media("screen"))._head().write("\n");
	}
	...
	protected void startTable(HtmlCanvas target, String cssclass, String width,
			String align, String border, String cellpadding, String cellspacing)
			throws IOException {
		target.table(
				class_(cssclass).width(width).align(align).border(border)
	...
	protected void writeTableHead(HtmlCanvas target, String cssclass,
			String width, String rowspan, String colspan, String content,
			String link) throws IOException {
		target.th(
				class_(cssclass).width(width).rowspan(rowspan).colspan(colspan))
				.write("\n");
		if (!link.equals(""))
			target.a(href(link));
		else
			target.a();
		target.write(content)._a()._th().write("\n");
	}

	protected void writeTableCellLink(HtmlCanvas target, String cssclass,
			String width, String rowspan, String colspan, String content,
			String link) throws IOException {
		target.td(
				class_(cssclass).width(width).rowspan(rowspan).colspan(colspan))
				.write("\n").a(href(link)).write(content)._a()._td()
				.write("\n");
	}

	protected void writeTableCellLiteral(HtmlCanvas target, String cssclass,
			String width, String rowspan, String colspan, String content)
			throws IOException {
		target.td(
				class_(cssclass).width(width).rowspan(rowspan).colspan(colspan))
	...
	protected void writeParagraph(HtmlCanvas target, String cssclass,
			String content, String link) throws IOException {
		target.p(class_(cssclass)).write("\n");
		if (!link.equals(""))
			target.a(href(link));
		else
			target.a();
		target.write(content)._a()._p().write("\n");
	}
