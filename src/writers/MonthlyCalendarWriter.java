...
public class MonthlyCalendarWriter extends TivooWriter {
	...
	public void writeEvents(HtmlCanvas summary, List<TivooEvent> eventlist,
			String summarypath) throws IOException {
		startTable(summary, "", "90%", "center", "1", "0", "0");
		DateTime current = TivooTimeUtils.createLocalTime(eventlist.get(0)
				.getStart());
		clearList();
		boolean flag = true;
		int count = 0;
		for (TivooEvent e : eventlist) {
			...
			if (!event.isEmpty() && (!flag || count == eventlist.size())) {
				startRow(summary);
				String header = TivooTimeUtils.createLocalTime(
						event.get(0).getStart()).toString("MMMM YYYY");
				...
				endRow(summary);
				DateTime currentMonth = TivooTimeUtils.createLocalTime(event
						.get(0).getStart());
				DateTime monthStart = currentMonth.minusDays(currentMonth
						.getDayOfMonth() - 1);
				int startWeek = monthStart.getWeekOfWeekyear();
				DateTime weekStart = monthStart.minusDays(monthStart
						.getDayOfWeek() - 1);
				for (int i = 0; i < 5; i++) {
					startRow(summary);
					String rowhead = weekStart.plusDays(i * 7)
							.toString("MM.dd")
							+ "-"
					...
					for (int j = 0; j < 7; j++) {
						...
						for (int k = 0; k < date.size(); k++) {
							if (date.get(k) == j
									&& week.get(k) == i + startWeek) {
								TivooEvent f = event.get(k);
								String detailpath = buildDetailPathRel(
										eventlist, f, summarypath);
								writeParagraph(summary, "", event.get(k)
										.getTitle(), detailpath);
								new DetailPageWriter().doWriteDetail(
										f,
										buildDetailPathAbs(summarypath,
												detailpath));
							}
						}
						summary._td();
