...
public class ConflictingEventsWriter extends TivooWriter {
	...
	protected void writeEvents(HtmlCanvas summary, List<TivooEvent> eventlist,
		...
		for (TivooEvent e : eventlist) {
			...
			writeTableCellLink(summary, "conflict", null, "1", "1",
					e.getTitle(), detailpath);
			endRow(summary);
			...
			new DetailPageWriter().doWriteDetail(e,
					buildDetailPathAbs(summarypath, detailpath));
		}
	}

	private List<TivooEvent> getConflicts(TivooEvent e,
			List<TivooEvent> eventlist) {
