...
public class GoogleCalParser extends TivooParser {
	...
	private List<DateTime> parseOneTimeEvent(String timestring) {
		...
		String month = splitted[2], date = splitted[3].replaceAll(",", ""), year = splitted[4], starttime = splitted[5], endtime = splitted[7];
		...
		DateTimeFormatter hourformat = new DateTimeFormatterBuilder().append(
				null, parsers).toFormatter();
		int _starthour = hourformat.parseDateTime(starttime).getHourOfDay();
		int _startminute = hourformat.parseDateTime(starttime)
				.getMinuteOfHour();
		...
		DateTime start = new DateTime(_year, _month, _date, _starthour,
				_startminute, timezone);
		DateTime end = new DateTime(_year, _month, _date, _endhour, _endminute,
				timezone);
	...
	private void parseRecurringEvent(String timestring) {
		...
		DateTimeFormatter formatter = DateTimeFormat.forPattern(
				"YYYY-MM-ddHH:mm:ss").withZone(timezone);
		...
		recurringstartend.add(recurringstart);
		recurringstartend.add(recurringend);
	}
	...
	private class TopLevelHandler implements ElementHandler {
		...
		public void onEnd(ElementPath elementPath) {
			timezone = DateTimeZone.forID(elementPath.getCurrent()
